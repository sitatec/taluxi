...
import 'package:taluxi/services/real_time_location/exceptions/device_location_handler_exception.dart';
...
void main() {
  ...
  final fakeCoordinate = Coordinate(latitude: -13.045356, longitude: 12.546447);
  final fakeLocationData = LocationData.fromMap({
    'latitude': fakeCoordinate.latitude,
    'longitude': fakeCoordinate.longitude,
    'accuracy': 0.0,
    'altitude': 0.0,
    'speed': 0.0,
    'speed_accuracy': 0.0,
  ...
  setUp(() {
    ...
    deviceLocationHandler = DeviceLocationHandler(location: location);
  ...
  test('Should get the current location of the device', () async {
    ...
    await deviceLocationHandler.initialize();
    expect(await deviceLocationHandler.getCurrentCoordinate(), fakeCoordinate);
  });

  test(
      'Should throw a [DeviceLocationHandlerException.locationServiceUninitialized()] when trying to get the current coordinate without initializing the device location handler object.',
      () {
    expect(
      () async => await deviceLocationHandler.getCurrentCoordinate(),
      throwsA(
        isA<DeviceLocationHandlerException>().having(
          (e) => e.exceptionType,
          'Exception type',
  ...
  test(
      ...
      () {
    expect(
      () async => await deviceLocationHandler.getCoordinatesStream(),
      throwsA(
        isA<DeviceLocationHandlerException>().having(
          (e) => e.exceptionType,
          'Exception type',
